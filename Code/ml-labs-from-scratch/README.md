...
Most people who finish Andrew Ng's Machine Learning Specialization can explain gradient descent on a whiteboard but have never actually written it in a loop. I wanted to close that gap. This folder is where I re-implement every core ML algorithm from the course — linear regression, logistic regression, neural networks — using **only NumPy**, no `sklearn.fit()`, no shortcuts.
